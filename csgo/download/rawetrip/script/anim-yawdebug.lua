...
function note_c.new(name)
    if (package.AnimYawNotes == nil) then 
        package.AnimYawNotes = {}
    end
...
function note_c:get(func)
    ...
    for key, value in note_c.sort(package.AnimYawNotes) do
        if (value == true) then
            index = index + 1
...
function note_c:set_state(value)
    package.AnimYawNotes[self.name] = value

    table.sort(package.AnimYawNotes)
end

--- @return: void
function note_c:unload()
    if (package.AnimYawNotes[self.name] ~= nil) then 
        package.AnimYawNotes[self.name] = nil 
    end
end
--- @endregion
...
--- @region: unnamed
--- @element: script vars
local script_name = "AnimYaw"
local script_type = "debug"
local script_clan_tag = "AnimYaw Debug"
...
ui_global.discord_link = menu_manager_c.new("Global", "Second", "Export Discord Link To Console"):button(function()
    local link = ("| Join our discord."):gradient({144, 238, 144, 255}, {0, 128, 0, 255})
    console_c.log("https://discord.gg/YzfRnZx83W" .. link)
end)

--- @group: Anti-Aim
local ui_anti_aim = {}

ui_anti_aim.switch = menu_manager_c.new("Anti-Aim", "Second", "Enable Anti-Aim"):checkbox()
...
ui_anti_aim.antibruteforce = menu_manager_c.new("Anti-Aim", "Second", "Anti BruteForce", true, ui_anti_aim.switch_condition):checkbox()
...
ui_Visual.switch = menu_manager_c.new("Visual", "Second", "Enable Visual"):checkbox()
ui_Visual.switch_condition = function() return ui_Visual.switch:get() end

ui_Visual.main_color = menu_manager_c.new("Visual", "First", "Main Color", true, ui_Visual.switch_condition):color_picker()
ui_Visual.first_color = menu_manager_c.new("Visual", "First", "First Color", true, ui_Visual.switch_condition):color_picker()
ui_Visual.second_color = menu_manager_c.new("Visual", "First", "Second Color", true, ui_Visual.switch_condition):color_picker()

ui_Visual.glow_switch = menu_manager_c.new("Visual", "First", "Enable Glow", true, ui_Visual.switch_condition):checkbox()
ui_Visual.glow_switch_condition = function() return ui_Visual.switch_condition() and ui_Visual.glow_switch:get() end
ui_Visual.glow_color = menu_manager_c.new("Visual", "First", "Glow Color", true, ui_Visual.glow_switch_condition):color_picker()

ui_Visual.sty_seperator = menu_manager_c.new("Visual", "First", "    "):label()
ui_Visual.style = menu_manager_c.new("Visual", "First", "Windows style", true, ui_Visual.switch_condition):combo("Default", "Alternative")

ui_Visual.watermark = menu_manager_c.new("Visual", "Second", "Enable Watermark", true, ui_Visual.switch_condition):checkbox()
ui_Visual.keybinds = menu_manager_c.new("Visual", "Second", "Enable Keybinds", true, ui_Visual.switch_condition):checkbox()
ui_Visual.indicators = menu_manager_c.new("Visual", "Second", "Enable Crosshair Indicators", true, ui_Visual.switch_condition):checkbox()

ui_Visual.hit_log = menu_manager_c.new("Visual", "Second", "Enable On Screen Shot Log", true, ui_Visual.switch_condition):checkbox()
ui_Visual.hit_log_condition = function() return ui_Visual.switch_condition() and ui_Visual.hit_log:get() end
ui_Visual.manage_color = menu_manager_c.new("Visual", "Second", "Manage Colors", true, ui_Visual.hit_log_condition):checkbox()
ui_Visual.manage_color_condition = function() return ui_Visual.hit_log_condition() and ui_Visual.manage_color:get() end
ui_Visual.hit_seperator = menu_manager_c.new("Visual", "First", "   "):label()
ui_Visual.hit_color = menu_manager_c.new("Visual", "First", "Hit Color", true, ui_Visual.manage_color_condition):color_picker()
ui_Visual.miss_color = menu_manager_c.new("Visual", "First", "Miss Color", true, ui_Visual.manage_color_condition):color_picker()

ui_Visual.seperator = menu_manager_c.new("Visual", "Second", "  ", ui_Visual.manage_color_condition):label()

ui_Visual.clan_tag = menu_manager_c.new("Visual", "Second", "Enable Clantag Spammer", true, ui_Visual.switch_condition):checkbox()
menu.set_callback("checkbox", ("%s_%s"):format("Enable Clantag Spammer - Visual", "checkbox"), ui_Visual.clan_tag.reference, function(new_value)
...
function condition_anti_aim.create_items()
    ...
    for key, value in ipairs(items) do
        ...
        setup.body_lean = menu_manager_c.new("Anti-Aim", "Second", name("Body Lean"), true, function() return visible_condition() and setup.desync_type:get() ~= 0 end):slider(1, 100)
        setup.inv_body_lean = menu_manager_c.new("Anti-Aim", "Second", name("Inverted Body Lean"), true, function() return visible_condition() and setup.desync_type:get() ~= 0 end):slider(1, 100)
    end
...
-- @item: Anti BruteForce

local function antibruteforce(shot_info)

    if not engine.is_in_game() then return end
    if not entitylist.get_local_player():is_alive() then return end
    
    local antibrute_info = { switch = ui.get_keybind_state(keybinds.flip_desync) }
    
    if antibrute_info.switch then 
    ui.set_keybind_state(keybinds.flip_desync, false) 
    end
    
    if not antibrute_info.switch then 
...
function watermark_c.handle()
    watermark_c.note:set_state(ui_Visual.watermark:get() and ui_Visual.switch:get())
    watermark_c.note:get(function(id)
        local f_r, f_g, f_b = color_c.unpack(ui_Visual.first_color:get())
        local s_r, s_g, s_b = color_c.unpack(ui_Visual.second_color:get())
        local a_r, a_g, a_b = color_c.unpack(ui_Visual.main_color:get())
        local g_r, g_g, g_b = color_c.unpack(ui_Visual.glow_color:get())
        ...
        if (ui_Visual.glow_switch:get()) then
            render.rect_shadow_ex(vector_2d(x + 1, y + 1), vector_2d(width - 2, height - 2), 16, vector(1, 22, 2), 4, color_c.new(g_r, g_g, g_b), 0, -1)
        end

        render_c.container(x, y, width, height, color_c.new(a_r, a_g, a_b), ui_Visual.style:get() + 1)

        render.text_gradient(fonts.verdana.default, x + space + 1, y + (height / 2) - (script_size.y / 2) + y_add[ui_Visual.style:get()], color_c.new(f_r, f_g, f_b), color_c.new(s_r, s_g, s_b), script_name, 0, true)

        render_c.multitext(fonts.verdana.default, x + script_size.x + space + 2, y + (height / 2) - (text_size.y / 2) + y_add[ui_Visual.style:get()], text, 1, true)
    end)
end
...
function indicators_c.handle()
    if (not ui_Visual.indicators:get() or not ui_Visual.switch:get()) then
        return
    end
...
function keybinds_c.handle()
    if (not ui_Visual.keybinds:get() or not ui_Visual.switch:get()) then
        return
    end
    ...
    if (alpha > 0.1 and ui_Visual.glow_switch:get()) then
        render.rect_shadow_ex(vector_2d(x + 1, y + 1), vector_2d(width - 2, height - 2), 16, vector(1, 22, 2), 4*alpha, color_c.new(g_r, g_g, g_b), 0, -1)
    end

    render_c.container(x, y, width, height, color_c.new(a_r, a_g, a_b, 255*alpha), ui_Visual.style:get() + 1)
...
function hit_log_c.shot_event(event)
    if (event.result ~= "Hit") then
        local color_miss = ui_Visual.manage_color:get() and ui_Visual.miss_color:get() or color_c.new(255, 150, 150)
    ...
    local color_hit = ui_Visual.manage_color:get() and ui_Visual.hit_color:get() or color_c.new(47, 255, 72)
...
function hit_log_c.handle()
    if (not ui_Visual.hit_log:get() or not ui_Visual.switch:get()) then
        return
    end
    ...
    for key, value in ipairs(hit_log_c.list) do
        ...
        if (value.alpha > 0.1 and ui_Visual.glow_switch:get()) then
...
local function on_load()
    ...
    local user_name = engine.get_gamename():gradient({135, 206, 250, 255}, {135, 206, 250, 255})
    local author = ("lordvortex#0143 & iLight#9470 "):gradient({135, 206, 250, 255}, {135, 206, 250, 255})
    local type = (script_type:gsub("^%l", string.upper)):gradient({135, 206, 250, 255}, {135, 206, 250, 255})
    local version = script_version:gradient({0135, 206, 250, 255}, {135, 206, 250, 255})
